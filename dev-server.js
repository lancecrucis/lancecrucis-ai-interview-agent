...
function buildFollowUpPrompt(history, summary, topics, state) {
  ...
  return `Technical interview in progress.\n\n${conv}\n\nCandidate: ${summary}\n\nTopics:\n${topicList}\n\nCovered: ${coveredCount} of ${topics.length}. Questions: ${userCount}. Questions remaining: ~${Math.max(0, 12 - userCount)} more. Difficulty: ${state.difficulty}\n\nFocus NEXT on: ${next}\n${adaptiveInstructions}\n\nFLOW RULES — Make it feel like a real interview:\n- Sometimes go DEEP on an answer (ask "why", "how would you handle edge cases", "what if...")\n- Sometimes move QUICKLY to the next topic if the answer is thorough\n- Vary your response length: sometimes 1 sentence, sometimes 3-4 sentences\n- If the answer is shallow or vague, PROBE: "Can you elaborate on that?"\n- If the answer is strong, acknowledge briefly and move on\n- Occasionally reference something they said earlier\n- Be warm and human. Use phrases like "Interesting", "Good point"\n- If struggling, say "No worries, let me rephrase that"\n- Track remaining questions and occasionally tell the user: "I have about X more questions"\n- Do NOT ask "What do you know about X?" — use scenario-based or "why" questions\n- When done, say a warm goodbye and append [DONE]\n- Do NOT say [DONE] until you have covered at least 4 topics AND asked at least 8 questions`;
}
